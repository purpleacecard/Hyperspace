...
//Retrieves an array of items with the category trending from the products.js file
let trending = products.filter((product, index) => {
...
trending.forEach((item, index) => {
  ...
  addToCartButton[index].id = trending[index].id;
});
...
plusButton.forEach((btn, index) => {
  const quantityLimit = trending[index].quantityLeft;
...
function cartPusher(index) {
  cart.push({
    ...
    priceCents: trending[index]['priceCents'],
    priceKobo: trending[index]['priceKobo'],
    quantity: Number(counterNum[index].innerText),
    imageLocation: trending[index]['imageLocation'],
  });
}
