...
//Retrieves an array of items with the category men-clothing from the products.js file
let menClothing = products.filter((product, index) => {
...
menClothing.forEach((item, index) => {
  ...
  addToCartButton[index].id = menClothing[index].id;
});
...
plusButton.forEach((btn, index) => {
  const quantityLimit = menClothing[index].quantityLeft;
...
function cartPusher(index) {
  cart.push({
    id: menClothing[index]['id'],
    name: menClothing[index]['name'],
    category: menClothing[index]['category'],
    priceCents: menClothing[index]['priceCents'],
    priceKobo: menClothing[index]['priceKobo'],
    quantity: Number(counterNum[index].innerText),
    imageLocation: menClothing[index]['imageLocation'],
  });
}
