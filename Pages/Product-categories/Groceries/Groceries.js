...
//Retrieves an array of items with the category groceries from the products.js file
let groceries = products.filter((product, index) => {
...
groceries.forEach((item, index) => {
  ...
  addToCartButton[index].id = groceries[index].id;
});
...
plusButton.forEach((btn, index) => {
  const quantityLimit = groceries[index].quantityLeft;
...
function cartPusher(index) {
  cart.push({
    ...
    priceCents: groceries[index]['priceCents'],
    priceKobo: groceries[index]['priceKobo'],
    quantity: Number(counterNum[index].innerText),
    imageLocation: groceries[index]['imageLocation'],
  });
}
