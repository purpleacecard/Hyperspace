...
//Retrieves an array of items with the category mobile-phones from the products.js file
let mobilePhones = products.filter((product, index) => {
...
mobilePhones.forEach((item, index) => {
  ...
  addToCartButton[index].id = mobilePhones[index].id;
});
...
plusButton.forEach((btn, index) => {
  const quantityLimit = mobilePhones[index].quantityLeft;
...
function cartPusher(index) {
  cart.push({
    id: mobilePhones[index]['id'],
    name: mobilePhones[index]['name'],
    category: mobilePhones[index]['category'],
    priceCents: mobilePhones[index]['priceCents'],
    priceKobo: mobilePhones[index]['priceKobo'],
    quantity: Number(counterNum[index].innerText),
    imageLocation: mobilePhones[index]['imageLocation'],
  });
}
