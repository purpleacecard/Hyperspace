...
//Retrieves an array of items with the category skincare from the products.js file
let skincare = products.filter((product, index) => {
...
skincare.forEach((item, index) => {
  ...
  addToCartButton[index].id = skincare[index].id;
});
...
plusButton.forEach((btn, index) => {
  const quantityLimit = skincare[index].quantityLeft;
...
function cartPusher(index) {
  cart.push({
    id: skincare[index]['id'],
    name: skincare[index]['name'],
    category: skincare[index]['category'],
    priceCents: skincare[index]['priceCents'],
    priceKobo: skincare[index]['priceKobo'],
    quantity: Number(counterNum[index].innerText),
    imageLocation: skincare[index]['imageLocation'],
  });
}
