...
//Retrieves an array of items with the category gaming from the products.js file
let gaming = products.filter((product, index) => {
...
gaming.forEach((item, index) => {
  ...
  addToCartButton[index].id = gaming[index].id;
});
...
plusButton.forEach((btn, index) => {
  const quantityLimit = gaming[index].quantityLeft;
...
function cartPusher(index) {
  cart.push({
    id: gaming[index]['id'],
    name: gaming[index]['name'],
    category: gaming[index]['category'],
    priceCents: gaming[index]['priceCents'],
    priceKobo: gaming[index]['priceKobo'],
    quantity: Number(counterNum[index].innerText),
    imageLocation: gaming[index]['imageLocation'],
  });
}
