...
//Retrieves an array of items with the category sneakers from the products.js file
let sneakers = products.filter((product, index) => {
...
sneakers.forEach((item, index) => {
  ...
  addToCartButton[index].id = sneakers[index].id;
});
...
plusButton.forEach((btn, index) => {
  const quantityLimit = sneakers[index].quantityLeft;
...
function cartPusher(index) {
  cart.push({
    ...
    category: sneakers[index]['category'],
    priceCents: sneakers[index]['priceCents'],
    priceKobo: sneakers[index]['priceKobo'],
    quantity: Number(counterNum[index].innerText),
    imageLocation: sneakers[index]['imageLocation'],
  });
}
