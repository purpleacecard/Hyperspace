...
//Retrieves an array of items with the category home-appliances from the products.js file
let homeAppliances = products.filter((product, index) => {
...
homeAppliances.forEach((item, index) => {
  ...
  addToCartButton[index].id = homeAppliances[index].id;
});
...
plusButton.forEach((btn, index) => {
  const quantityLimit = homeAppliances[index].quantityLeft;
...
function cartPusher(index) {
  cart.push({
    id: homeAppliances[index]['id'],
    name: homeAppliances[index]['name'],
    category: homeAppliances[index]['category'],
    priceCents: homeAppliances[index]['priceCents'],
    priceKobo: homeAppliances[index]['priceKobo'],
    quantity: Number(counterNum[index].innerText),
    imageLocation: homeAppliances[index]['imageLocation'],
  });
}
