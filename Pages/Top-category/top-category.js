...
//Retrieves an array of items with the category top-category from the products.js file
let topCategory = products.filter((product, index) => {
  if (product.category === 'top-category') {
    return true;
  }
});
...
topCategory.forEach((item, index) => {
  ...
  addToCartButton[index].id = topCategory[index].id;
});
...
plusButton.forEach((btn, index) => {
  const quantityLimit = topCategory[index].quantityLeft;
...
function cartPusher(index) {
  cart.push({
    id: topCategory[index]['id'],
    name: topCategory[index]['name'],
    category: topCategory[index]['category'],
    priceCents: topCategory[index]['priceCents'],
    priceKobo: topCategory[index]['priceKobo'],
    quantity: Number(counterNum[index].innerText),
    imageLocation: topCategory[index]['imageLocation'],
  });
}
