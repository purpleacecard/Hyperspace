...
//Retrieves an array of items with the category new-to-hyperspace from the products.js file
let newToHyperspace = products.filter((product, index) => {
  if (product.category === 'new-to-hyperspace') {
    return true;
  }
});
// console.log(newToHyperspace);
...
newToHyperspace.forEach((item, index) => {
  ...
  addToCartButton[index].id = newToHyperspace[index].id;
});
...
plusButton.forEach((btn, index) => {
  const quantityLimit = newToHyperspace[index].quantityLeft;
...
function cartPusher(index) {
  cart.push({
    id: newToHyperspace[index]['id'],
    name: newToHyperspace[index]['name'],
    category: newToHyperspace[index]['category'],
    priceCents: newToHyperspace[index]['priceCents'],
    priceKobo: newToHyperspace[index]['priceKobo'],
    quantity: Number(counterNum[index].innerText),
    imageLocation: newToHyperspace[index]['imageLocation'],
  });
}
